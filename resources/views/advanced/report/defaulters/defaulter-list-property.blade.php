...
<div class="content">
  ...
  <div id="card">
    ...
    <div class="row" style="border-bottom: 2px solid black;">
      <h4 style="text-align: center; font-size: 14px; color: black; font-weight: 600;">Property Defaulters List Grouped By Electoral Area for <?= ucwords($year); ?></h4>
    </div>
  <table id="fBill1" class="display" cellspacing="0" width="100%">
      ...
      <tbody>
          @foreach($zonals as $electoral)
          <div class="tableInner">
            <tr class="odd2 heyy">
                <td>
                  <a href="{{route('report.defaulters.details', ['type' => 'property', 'code' => $electoral->code, 'operator' => $operator, 'location' => $location, 'year' => $year, 'amount' => $amount])}}"><img src="/advanced/1/add-square-button.png"></a>
                </td>
                <td>
                  <a style="color:blue; font-weight: 400;" href="{{route('report.defaulters.details', ['type' => 'property', 'code' => $electoral->code, 'operator' => $operator, 'location' => $location, 'year' => $year, 'amount' => $amount])}}">
        ...
        <div class="tableInner">
            <tr class="odd2 heyy">
                ...
                <td class="text-number">
                  ...
                  <?php
                    $sumArrears = 0.0;
                    foreach ($zonals as $key => $electoral) {
                      $sumArrears += $electoral->bills->sum('arrears');
                    };
                ...
                <td class="text-number">
                  ...
                  <?php
                    $sumArrears = 0.0;
                    foreach ($zonals as $key => $electoral) {
                      $sumArrears += $electoral->bills->sum('current_amount');
                    };
                    echo \App\Repositories\ExpoFunction::formatMoney($sumArrears, true);
                  ?>
                </td>
                <td class="text-number">
                  ...
                  <?php
                    $sumArrears = 0.0;
                    foreach ($zonals as $key => $electoral) {
                      $sumArrears += ($electoral->bills->sum('arrears') + $electoral->bills->sum('current_amount'));
                    };
                    echo \App\Repositories\ExpoFunction::formatMoney($sumArrears, true);
                  ?>
                </td>
                <td class="text-number">
                  ...
                  <?php
                    $sumArrears = 0.0;
                    foreach ($zonals as $key => $electoral) {
                      $sumArrears += $electoral->bills->sum('total_paid');
                    };
                    echo \App\Repositories\ExpoFunction::formatMoney($sumArrears, true);
                  ?>
                </td>
                <td class="text-number">
                  ...
                  <?php
                    $sumArrears = 0.0;
                    foreach ($zonals as $key => $electoral) {
                      $sumArrears += ($electoral->bills->sum('arrears') + $electoral->bills->sum('current_amount')) - $electoral->bills->sum('total_paid');
                    };
                    echo \App\Repositories\ExpoFunction::formatMoney($sumArrears, true);
                  ?>
                </td>
  ...
  {{$zonals->appends(request()->query())->links()}}
