    ...
    public function collection()
    {
        ...
        switch ($type) {
            ...
            case 'zonals':
                ...
                break;
            case 'category':
                $bills = PropertyCategoryReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('p'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('p'))->orderBy('account_no', 'asc');
                }])->first()->bills;
                break;
            case 'type':
                $bills = PropertyTypeReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('p'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('p'))->orderBy('account_no', 'asc');
                }])->first()->bills;
