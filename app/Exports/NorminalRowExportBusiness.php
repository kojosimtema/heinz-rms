...
use App\Reports\ZonalBusinessReport;
    ...
    public function collection()
    {
        ...
        switch ($type) {
            case 'electoral':
                ...
                break;
            case 'community':
                $bills = CommunityBusinessReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'))->orderBy('account_no', 'asc');
                }])->first()->bills;
                break;
            case 'zonal':
                $bills = ZonalBusinessReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'))->orderBy('account_no', 'asc');
                }])->first()->bills;
                break;
            case 'category':
                $bills = BusinessCategoryReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'))->orderBy('account_no', 'asc');
                }])->first()->bills;
                break;
            case 'type':
                $bills = BusinessTypeReport::where('code', $this->electoral)->whereHas('bills', function ($q) use ($year) {
                    $q->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'));
                })->with(['bills' => function ($query) use ($year) {
                    $query->where('year', $year)->where(strtoupper('bill_type'), strtoupper('b'))->orderBy('account_no', 'asc');
                }])->first()->bills;
